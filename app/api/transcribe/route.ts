...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log('API Key check:', {
      ...
      prefix: GOOGLE_API_KEY ? GOOGLE_API_KEY.substring(0, 5) + '...' : 'undefined',
    });
    ...
    const apiRequest = {
      config: {
        ...
        speechContexts: [{          // Add some context for legal terminology
          phrases: [
            "legal", "contract", "agreement", "clause", "plaintiff", "defendant",
            "lawyer", "attorney", "court", "judge", "law", "provision",
            "meeting", "conference", "call", "discussion", "conversation"
          ],
          boost: 10  // Boost recognition of these phrases
        }],
        // Audio settings to improve recognition
        audioChannelCount: 1,        // Mono audio is typical for meetings
      },
      audio: {
        content: audioBase64
      }
    ...
    if (!response.ok) {
      ...
      try {
        ...
        console.error('Google Speech API error response:', JSON.stringify(errorDetails, null, 2));
      } catch (parseError) {
        ...
        try {
          ...
          errorDetails = { error: 'Unknown error', httpStatus: response.status };
        }
      }
    ...
    console.log('Google Speech API full response:', JSON.stringify(data, null, 2));
    console.log('Google Speech API response structure:', JSON.stringify({
    ...
    if (data.results && Array.isArray(data.results) && data.results.length > 0) {
      try {
        ...
        emptyAlternativesFound = data.results.some(
          (result: any) => result.alternatives && 
        ...
        if (emptyAlternativesFound) {
          console.log('Empty alternatives found in the response - audio was processed but no speech detected');
        }
        
        const transcriptParts = data.results
          .map((result: any) => {
            if (result && result.alternatives && Array.isArray(result.alternatives) && 
                result.alternatives.length > 0 && result.alternatives[0].transcript) {
    ...
    if (transcription && transcription.trim() !== '') {
      ...
      const diagnosticInfo = {
        ...
        apiKeyFirstChars: GOOGLE_API_KEY ? GOOGLE_API_KEY.substring(0, 5) + '...' : 'missing'
      };
    ...
    try {
      await fetch('http://localhost:3000/api/latest-transcript', {
        method: 'POST',
    ...
    try {
      if (data && 
        ...
        if (Array.isArray(data.results[0].alternatives[0].words) &&
    ...
    return NextResponse.json({
      ...
      message: transcription ? 'Speech transcribed successfully' : 
               (emptyAlternativesFound ? 'No speech detected in audio' : 'Failed to transcribe speech')
    }, { headers });
  } catch (error: unknown) {
